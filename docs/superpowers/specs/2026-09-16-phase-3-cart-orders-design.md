...
| C3 | unsorted product locks (`CartService.listItemsForCheckout` with no `orderBy`) | Attempted 3 consecutive runs; no deadlock reproduced in any run — all 3 passed `{ 201: 20 }`, A = B = 30, conserved. `CheckoutService.checkout` independently re-sorts `sortedItems` by `productId` before decrementing (its own defensive re-sort, documented in `checkout.service.ts`), so removing the sort from `CartService` alone does not unfix the lock order for this call path; the deadlock this control targets was not observed | 20 × 201, zero 500s, A = B = 30, conserved |
