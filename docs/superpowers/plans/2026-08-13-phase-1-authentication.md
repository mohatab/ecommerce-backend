...
**BLOCKER for Phase 2 — `RolesGuard` and admin bootstrap must ship together:** nothing in this phase can create an `ADMIN`. Registration hardcodes `CUSTOMER` and no other code path writes `role`. Phase 2 must therefore deliver a seed script or one-off promotion path **in the same change as `RolesGuard`**, not after it. A guard shipped alone protects routes that no real account can reach: it is exercisable only via `createUser(prisma, { role: Role.ADMIN })` in tests, which is exactly the correct-by-inspection-but-unproven pattern that deferring the guard out of Phase 1 was meant to avoid — and it would leave the live deployed API, a foundation-spec success criterion, with no administrator. This is a Phase 2 entry condition, not a follow-up.
